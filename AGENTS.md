...
**The JS surface is declared, not reached into.** `window.sassy.compile()`, `.reload()`, `.poll()`, `.capture()` and `.push()`, plus `sassy:before-compile`, `sassy:compiled`, `sassy:reload`, `sassy:captured` and `sassy:pushed` on `document`. The Oxygen Angular reach-in is gone, and a builder integration is now a listener that forwards the event into an iframe. `assets/js/sassy.js` renders diagnostics through `renderDiagnostic()`, which mirrors `Diagnostic::render()`.
...
**The paintbrush, tier 1.** Inspector edits mutate the CSSOM; Sassy snapshots its own sheets when they load (and again after every reload, since Live Compile and the poll swap a link's `href`), diffs on 🖌️ Capture, maps each change through the source map and shows the patch in the panel. The JS learns which sheets are Sassy's from `data-sassy-sheets` on the panel element, written by `print_errors()` at `wp_footer` 30, because the printers exist only after the head has printed. Declarations are diffed as the rule serialises them (`style.cssText`), which keeps a shorthand whole; the sheet's text is fetched as bytes so Dart's byte-order mark keeps its column; blocks in the text are aligned to flattened CSSOM rules by selector key (whitespace, quotes and `::` normalised, plus the four rewrites Chrome makes when it prints a selector: a qualifying `*` dropped, `odd`/`even` as `2n+1`/`2n`, keyframe `from`/`to` as `0%`/`100%`), walking both lists, and the baseline is keyed by selector occurrence rather than index, because Chrome's per-rule **+** inserts a new rule into the same sheet after the one it was pressed under. Such a rule reports as `new rule` with the rule above it and its source line as where it belongs, and Push writes it there as `@at-root`. Three limits, known and not worth their fix: alignment pairs within a window of eight blocks (four until a run of five stagger rules the key could not match stranded everything after them on a live sheet), so a new rule whose selector recurs in the text inside that window can pair with the later block and misattribute a line (a wrong line, never a wrong write, since the writer checks the value); a sheet recompiled elsewhere without a reload has text newer than the CSSOM until the reload re-baselines; and a new rule with the same selector as an earlier one inserted *above* it would swap occurrence numbers, which Chrome's styles pane cannot do. A written push consumes the capture: what was written is no longer a paint; when fewer than nine in ten rules pair, lines are withheld for that sheet rather than guessed. A rule in `inspector-stylesheet` and an `element.style` edit are listed copy-only too; past three `element.style` entries fold in the panel under a count, since scripts set most of them on a real page, and the copy text keeps them all. A rule deleted in the inspector, in the baseline and the text but no longer in the CSSOM, is reported at its block's line with the declarations it had and sent to Push as `{selector, remove: true}`; `Source_Writer::removal()` deletes the block, opener to closer, only when the opener line is exactly that selector and no line inside holds a `{`, so a nested source rule (whose compiled selector its opener does not carry) and anything with nested blocks refuse to the copy path. Copy yields the patch as text, `sassy:captured` carries it, and Logging → Capture diffs logs it. The panel is one `panel(title, blocks, kind)` for errors and captures alike, with `data-kind` for the stylesheet and a Copy beside Dismiss. The spike that gated this is `tests/fixtures/paintbrush-spike.html`.
...
- **SCSS** (root) — shows ❌ SCSS on compile errors
  ...
  - 🖌️ **Capture** — diff the live CSSOM against the snapshot and show the patch; **Push to source** appears in the panel when the write gate is open
  - 🖌️ **Push to source** — capture and push in one click; present only when the write gate is open
...
| `test-source-writer.php` | Every refusal: outside the graph, changed since the compile, the property boundary, a `$variable`, a served value the source lacks, no `;`, past the end, a shared-line removal, an addition on a line that does not open a block; and what applies: a compact line, a lone removal, an addition after a sibling declaration or after the opener with the block's indentation, an addition inside a hoisted `@supports` landing in that block, a new rule after its neighbour's block as `@at-root (without: all)` under its ancestor at-rules with the depth scan over strings, comments and interpolation, and the malformed refusals (newline, brace, non-name), two changes bottom-up, owner and inode kept |
...
| `sassy-write-source` | `current_user_can(SASSY_WRITE_SOURCE)` if the constant names a capability, else `false` | The gate for the paintbrush's push to source. Never implied by `sassy-dev`; the reference install binds it to `dev` |
...
| `wp_ajax_sassy_write` | `Sassy::write_source()`. `Policy::active()`, then `can_write_source()`, then the nonce, then POST only; `changes` as one JSON field. No `nopriv` registration |
...
| `SASSY_WRITE_SOURCE` | User's `wp-config.php` | A capability name; holders may push to source. The `sassy-write-source` default |
...
- `Import_Graph::stamp()` calls `clearstatcache(true, $path)` first: PHP 8.2 serves the last stat of a path from memory across a write and a touch in the same process, which would let the writer see the recorded stamp on a file it had just been asked about.
- A raised discovery context records `message (file:line)`, so the plugin whose callback raised can be found. The usual cause under WP-CLI is a plugin assigning its instance at file scope and reading it back with `global`, which `wp-settings.php` being included from a method defeats; `sassy.php` declares `global $Sassy` for exactly this reason.
- Where the web server owns `wp-content/scss`, a default ACL (`setfacl -d`) is what lets a developer account compile; `chmod 2775` does not survive php-fpm's umask and Sassy's in-place rewrites.
