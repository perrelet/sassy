...
**The source-map notice belongs to the `Printer`, not to Lightning.** Detect the observable condition, not a particular tool's behaviour: after the `sassy-css` filter, when source maps are on and a map was written but the returned CSS carries no `sourceMappingURL`, the `Printer` reports a `source: 'sassy'` notice naming the handle. That holds for any post-processor, which is why it belongs upstream of all of them. Lightning then *triggers* the canonical notice rather than emitting it, and the reporting channel above is earned separately, by Lightning's silent failures.

**Nothing in `Lightning_CSS_Postprocessor` strips a map link**, so do not go looking for the code that does. AGENTS.md says `lightningcss` drops the comment from its own output. **Treat that as unverified**: no binary is installed on this machine, `npx --no-install lightningcss` fails, and Sassy reports Lightning as off, so nobody has watched it happen here. The detection above does not depend on knowing, which is the point of specifying it as a condition. If you do get a binary in front of you, confirm the cause and write down what you saw.

**Say when registration happens, or every consumer gets it wrong once.** Integrations register their variables filter in a constructor called from `load_integrations()` on `after_setup_theme`, and deleting that method deletes the only registration point Sassy has. Replace it with a `sassy-register` action fired on `after_setup_theme`, receiving the registry, and document the real constraint alongside it: registration works from anywhere so long as it precedes the first compile, which is `style_loader_src` during `wp_head`. A provider that registers later does nothing at all and says nothing about it, and the cutoff is per asset rather than global: each `Printer` resolves its variables lazily, so a provider registered after `d-pace-frontend` compiles still applies to `d-pace-admin`. Say that in the docs, because "before the first compile" is the obvious wrong summary of it.
...
**A provider is a slug plus a callable**, and the slug is what `wp sassy status` prints. Without an identity there is nothing to list and the acceptance cannot be checked, so registration takes one: `register_variables('bricks', $callable)`, and so on for the other three kinds. Re-registering a slug replaces it, which is what makes a provider overridable by name.

**The two routes are not equal, and the brief should not pretend otherwise.** A registered post-processor can report diagnostics; a `sassy-css` callback cannot, because the filter's contract is a string in and a string out and changing that would break every existing caller. So raw filters keep working, stay unlisted, and stay unable to report. That is the price of "alongside, not instead", and it is worth stating in the docs rather than leaving someone to discover that their filter cannot warn about anything.
...
**`sassy-compiler` is a fifth extension point, and it stays outside the registry.** It hands out scssphp's own `Compiler` object before compilation, so it bypasses every typed contract and means nothing under Dart Sass. It is an engine-specific escape hatch, not a kind of provider: document it as one, and do not try to make the registry describe it.

**Lightning's three filters survive unchanged.** `sassy-lightning-css`, `sassy-lightning-css-binary` and `sassy-lightning-css-options` are documented, and `SASSY_LIGHTNINGCSS_BIN` and `SASSY_TOOLS_DIR` feed them. Porting Lightning to the registry is about giving it a way to report, not about relitigating how it is configured, and moving its options into a provider argument would be a second break for no gain. Phase 3 removed `sassy-src-map-options` because its whole value surface was another library's option names; that reasoning does not apply here.
...
- `tests/test-extensions.php`: registering each of the four kinds and seeing it take effect; a provider registered through the API appearing in the status listing by slug while a raw `add_filter` does not; re-registering a slug replacing it; a post-processor reporting a `Diagnostic` that reaches `Printer::get_warnings()`. Registration timing is per asset: a provider registered between two compiles applies to the second and not the first, and a test should pin that rather than assume a global cutoff.
...
Update `README.md`, `AGENTS.md` and `docs/upgrading-to-3.0.md` as part of landing. **README is the one that matters most here**: its `## Integrations` section documents all three builders to users, and a cross-reference in the variables section points readers at it. Deleting the features and leaving the readme selling them is worse than any stale internal note. The upgrade notes need the integrations' removal and the variables that go with them, which is the most visible break in 3.0 for anyone running Bricks or Oxygen. AGENTS.md's Integrations section describes three classes that will no longer exist.
