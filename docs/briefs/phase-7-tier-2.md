...
**Additions are refused; removals are applied only when the line is the declaration alone.** A `+ prop: value` has no old value to match and no exact place to go; it stays copy-only. A `- prop: value` deletes the line when, trimmed, the line is exactly `prop: value;`, else refuses. *Overtaken on 2026-09-14 after Jamie's walk: an addition does have an exact place, first inside the block the mapped line opens, and 3.5.0 writes it under the guard that the mapped line ends in `{`. The removal rule stands.*
