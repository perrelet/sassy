...
**Discovery reads scripts the way it reads styles, into a second store.** `Style_Stack::read_queues()` also reads the registries from a `sassy-script-queues` filter, defaulting to `[wp_scripts()]`, into `Asset`s with `type: 'script'`. **48 handles are shared** between a style and a script on the reference install today (`common`, `revisions`, `site-icon`, ...), so one store keyed by handle would lose them, and a store keyed by `type:handle` would break every caller that reads the key as the handle: `audit()`, `dependents_of()`, the CLI's handle filter in `discover()` and `find_style()` all do. So: two stores, both keyed by handle. `all($type = 'style')` returns one of them, or both under `'all'` keyed `type:handle`, which only the two listing surfaces ask for. `handle($h, $type = 'style')` follows the same default. `styles()` and `scripts()` name the two. `compilable()` is unchanged in shape and content: `is_compilable()` requires `scss`, so scripts never reach a `Printer`, `audit()` or `check`, and every existing caller keeps its keys. Isolation already copies `wp_scripts()`; nothing there changes.
...
`scope` also records the attribute names it saw, as the attribute is written in markup: `setAttribute('data-theme'` yields `data-theme`, and `dataset.theme` yields `data-theme` too, `dataset.fooBar` yielding `data-foo-bar` by the DOM's own camelCase rule; `classList` yields nothing, a class is not an attribute. `--touches` accepts the name with or without the `data-` prefix. That list is what phase 9 will cross-reference. Add `.style =` and `.style.cssText =` beside the per-property shape: assigning the whole attribute is an inline write too.

**Only JavaScript has a surface.** All 263 local scripts on the reference install are `.js`, and a `.php` registered as a script (core's TinyMCE loader is one) would report its PHP source. `Style_Surface::of()` answers only for `js` and `mjs`; anything else has no surface and says why.
...
**Cache by stamp, in one record.** 14 MB of regex per `list --type=script` is a second or two on the CLI and too much on every dashboard load. `Style_Surface` keeps one transient, `sassy-surfaces`, mapping path to `[stamp, counts, attributes]` with `Import_Graph::stamp()` as the stamp, recomputes only entries whose stamp moved, and `Compile_Cache::forget_all()` drops it: the SQL branch already matches it by pattern, the external-object-cache branch must delete it by name, since it is not a handle in the index. That branch is the one staging runs. The first dashboard load after a deploy pays the full scan once; say so in AGENTS.md rather than let someone profile it.
...
- `tests/test-style-stack.php`: scripts are discovered from `wp_scripts()` and from a `sassy-script-queues` queue; a style and a script sharing a handle are both kept and `handle()` answers by type; `all()` with no argument is exactly what it was, keyed by handle; `all('all')` holds both under `type:handle`; `compilable()` never contains a script; `styles()` and `scripts()` narrow. The bootstrap gains a `wp_scripts()` stub over a second `Sassy_Test_Queue`.
- `tests/test-style-surface.php`: one fixture per category and per "does not count" row above; `dataset.theme`, `dataset.fooBar` and `setAttribute('data-theme'` yield `data-theme`, `data-foo-bar` and `data-theme`; a minified one-liner is still counted; a remote script and a `.php` script have no surface; a recomputation happens only when the stamp moves; `forget_all()` drops the record under both branches; and `assets/js/sassy.js` reports exactly `['scope']`.
...
Update `AGENTS.md`, `README.md` and `docs/upgrading-to-3.0.md` as part of landing. README's WP-CLI section gains `--type` and `--touches`; the upgrade notes carry `all('all')` and the second store; AGENTS.md's rule that nothing outside `Style_Stack` reads `wp_styles()->registered` extends to `wp_scripts()`, and its counts (352, 337, 3 compilable) gain the scripts beside them.
