...
| Push that new rule | `✔ written   …_site-header.scss:<line>  @at-root header….site-header-inner { color: pink; }`; the file has a blank line then an `@at-root (without: all) { … }` block right after the `&-inner { … }` block, at its indentation; the compile succeeds and the page shows it. Revert with git afterwards |
| Press the per-rule `+` under a rule that sits inside a `@media`, add a declaration, Capture, Push | The report ends `in @media (min-width: 440px)`; the written block nests the `@media` inside the `@at-root`; the compiled CSS has the rule inside that query, and the page shows it only at that width. Revert |
